...
| 5 | **Schéma de catalogue v2** | `format` (texte/illustré), `nature` (fiction/médiation — sépare les romans des lectures de médiation culturelle à l'index ; prérequis de la séparation demandée par le chantier 8 de la [roadmap Conception](../conception/ROADMAP.md)), `variantOf` (groupement des éditions — condition de levée du moratoire), `wordCount`/`readingMinutes` (calculés par le script), `genre` (vocabulaire fermé, distinct des tags). `schemaVersion: 2` + validateur JS mis à jour de façon coordonnée ([`CATALOGUE.md`](CATALOGUE.md)). Valeurs exactes de `nature`, défaut implicite (`fiction` si absent, pour ne pas rétrofiter les 11 romans publiés) et comportement de l'index : à trancher dans ce chantier | ③ | §D.4.1 ; Pierre, 2026-08-13 | à faire |
| 6 | **Gouvernance des tags** | Vocabulaire contrôlé court (genre + format + 2-3 thèmes libres), documenté côté ateliers ; assainir les 30 tags actuels (12 orphelins) ; la distinction fiction/médiation culturelle ne passe pas par les tags mais par `nature` (chantier 5) — le tag posé par l'atelier médiation n'est que la solution d'attente | ① | §D.4.7 | à faire |
| 7 | **Catégories déclaratives affichées** | Badges calculés depuis le schéma v2 : durée de lecture, format, capacités interactives déclarées (codex, carte, choix, audio) — le sous-ensemble resserré de l'étude du champ des possibles §12 ; badge et filtre d'index fiction/médiation culturelle depuis `nature` | ①/③ | §D.4.1, étude MVP §12 | dépend de 5 |
