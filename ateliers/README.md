...
| [`mediation-culturelle/`](mediation-culturelle/WORKFLOW.md) | Lecture de **médiation culturelle** non romancée : un sujet réel du brief (lieu, personnage historique, métier, événement) documenté par recherche sourcée en amont de l'écriture, restitué en lecture explorable (parcours de médiation, notices de codex sourcées) ; ses livrables se signalent par le premier tag `médiation culturelle` (brief : [`BRIEF.md`](mediation-culturelle/BRIEF.md)) | v1 | expérimental (pilote à froid à programmer) | [`livres/_template/`](../livres/_template/README.md) (`atelier-liseuse v1`) | aucun encore |
